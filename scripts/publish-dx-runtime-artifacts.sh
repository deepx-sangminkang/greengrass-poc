...
#   dxrt-driver-dkms_2.4.1-2_all.deb  NPU Linux driver (DKMS), from dx_rt_npu_linux_driver
#   dx_rt.tar.gz                      dx_rt main branch + Ubuntu 26.04 cstdint patch
#   fw.bin                            firmware, from dx_fw m1/2.5.6/mdot2
...
DRIVER_DEB_PATH="${DX_DRIVER_DEB_PATH:-release/2.4.1/dxrt-driver-dkms_2.4.1-2_all.deb}"
DXRT_REPO="${DX_RT_REPO:-https://github.com/DEEPX-AI/dx_rt.git}"
DXRT_REF="${DX_RT_REF:-main}"
FW_REPO="${DX_FW_REPO:-https://github.com/DEEPX-AI/dx_fw.git}"
FW_PATH="${DX_FW_PATH:-m1/2.5.6/mdot2/fw.bin}"
...
echo "==> [1/4] dx_rt_npu_linux_driver: ${DRIVER_DEB_PATH}"
git clone --depth 1 "$DRIVER_REPO" driver
cp "driver/${DRIVER_DEB_PATH}" "$OUT/dxrt-driver-dkms_2.4.1-2_all.deb"

echo "==> [2/4] dx_rt: ${DXRT_REF} + Ubuntu 26.04 cstdint patch"
git clone --depth 1 --branch "$DXRT_REF" "$DXRT_REPO" dx_rt
# Ubuntu 26.04 patch: cxxopts.hpp needs an explicit <cstdint> to build under the
# newer GCC/libstdc++ that ships on Ubuntu 26.04 (matches the sed the
# com.deepx.dx-runtime recipe used to run on-device against the umbrella clone).
...
if [ -f "$CXXOPTS" ] && ! grep -q '#include <cstdint>' "$CXXOPTS"; then
  ...
  echo "    applied <cstdint> patch to $CXXOPTS"
else
  echo "    WARNING: cstdint patch not applied (file missing or already patched): $CXXOPTS" >&2
fi
rm -rf dx_rt/.git
tar czf "$OUT/dx_rt.tar.gz" dx_rt

echo "==> [3/4] dx_fw: ${FW_PATH}"
git clone --depth 1 "$FW_REPO" dx_fw
...
for f in dxrt-driver-dkms_2.4.1-2_all.deb dx_rt.tar.gz fw.bin dx_stream.tar.gz; do
